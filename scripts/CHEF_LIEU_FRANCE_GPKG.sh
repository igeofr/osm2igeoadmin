...
echo "Debut > CHEF_LIEU"
$LINK_OGR -progress -s_srs EPSG:2154 -t_srs EPSG:$OUT_EPSG -f 'GPKG' -update -append 'data_temp/'$PAYS/$OUT_EPSG'/OSM2IGEOADMIN.gpkg' 'data_temp/'$PAYS/$OUT_EPSG'/CHEF_LIEU_SELECT.vrt' -dialect SQLITE -sql "SELECT * FROM (
------------------------------------------------------------------------------------------------------------
------------------------------------ CHEF LIEU -------------------------------------------------------------
------------------------------------------------------------------------------------------------------------
SELECT
