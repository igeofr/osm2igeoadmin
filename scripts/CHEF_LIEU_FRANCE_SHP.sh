...
echo "Debut > CHEF_LIEU.shp"
$LINK_OGR -progress -s_srs EPSG:2154 -t_srs EPSG:$OUT_EPSG -f 'ESRI Shapefile' 'data_temp/'$PAYS/$OUT_EPSG'/CHEF_LIEU.shp' 'data_temp/'$PAYS/$OUT_EPSG'/CHEF_LIEU_SELECT.vrt' -dialect SQLITE -sql "SELECT * FROM (
------------------------------------------------------------------------------------------------------------
------------------------------------ CHEF LIEU -------------------------------------------------------------
------------------------------------------------------------------------------------------------------------
SELECT
...
-----------------------------------------
FROM point AS p, poly AS m WHERE p.name=m.name OR p.alt_name=m.name
------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------
)" -lco ENCODING=$ENCODAGE -lco SPATIAL_INDEX=YES --debug ON -skipfailures
echo "Fin > CHEF_LIEU.shp"
